...
| `final_gate_reviewer` | read-only | 任务结束前综合核对范围、批准记录、diff、测试、评审和安装证据 | 修改候选结果、替代缺失证据 | 禁止 |
...
`final_gate_reviewer` 只有在需求范围、批准的规格/计划、最新 diff、测试证据、任务级评审、本机安装验证和未解决项全部满足时才能输出 `APPROVED`。它不能把“已修改但未验证”当作通过。
