...
> 初始批准：2026-07-15
> 效率修订批准：2026-07-18
> 当前源码：`codex/agents/`、`scripts/codex_agents.py`、`scripts/validate_codex_agents.py`
...
- 项目级 `.codex/agents/`；
- 替换整个 `config.toml`；
- 管理全局模型、Provider、认证、MCP、插件、profile 或 Skill；
...
所有角色均不固定 `model`。只有以下边界明确的轻量只读角色固定：
...
架构、实现、测试、代码评审、数据一致性和最终门禁角色继承父会话，避免降低复杂任务能力或绑定单一 Provider。
...
| 描述过宽或角色重叠 | description 长度上限、职责矩阵、16 个路由案例、每个产物一个主要评审角色 |
