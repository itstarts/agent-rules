...
`install.sh` 把仓库内的 `AGENTS.md` 接入本机 Codex 配置。脚本只处理 Codex。

## 支持的平台

- macOS / Linux：默认支持 Bash 与 `ln -s`。
- Windows：Codex 的软链模式通常需要开发者模式或管理员权限。无法创建软链时，可准备本机补充文件，由脚本生成拼接文件。
...
| 模式 | 适用场景 | 同步方式 |
|------|----------|----------|
| 纯软链 | Codex 无本机补充 | `git pull` 更新仓库后自动生效 |
| 拼接文件 | Codex 存在本机补充 | 更新仓库或补充文件后重跑 `./install.sh` |

软链按绝对路径引用当前 clone。安装后请保留 clone 目录，不要移动或删除；换位置时重新 clone 并重跑安装脚本。
...
- 指向其它位置的软链：纯软链模式会备份；拼接写入模式会先移除旧软链再写入新文件，避免顺着软链覆盖源文件。
...
存在 Codex 本机补充时，脚本会生成“仓库源 + 本机补充”的拼接文件；仓库源或补充文件更新后需要重跑安装脚本。
...
删除软链或生成文件不会修改源仓库。确认不再需要该 clone 后，可以删除 clone 目录。
