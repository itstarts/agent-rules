...
本仓库把 `AGENTS.md` 作为唯一规则源，再用软链或拼接文件接入 Codex。
...
Codex 可直接读取 `AGENTS.md`。本仓库的安装脚本只负责把全局规则接入 Codex 的全局位置。
...
软链会随仓库更新自动生效；拼接文件需要重跑 `./install.sh`。
